...
class OpenLoopAgent(BaseAgent):
    ...
    def run_episode(self, env: TabletopManipulationEnv) -> EpisodeResult:
        ...
        return EpisodeResult(success=state.success, steps=steps, perturbation=env.perturbation, agent=self.name, seed=env.seed, true_failures=failures, event_log=env.events)
    ...
    def run_episode(self, env: TabletopManipulationEnv) -> EpisodeResult:
        ...
        while steps < self.config.max_steps and not state.success:
            ...
            if not result.action_succeeded:
                ...
                plan = deque(env.nominal_plan(state))
        failures = len({int(e["step"]) for e in env.events if e.get("failure")})
        return EpisodeResult(success=state.success, steps=steps, perturbation=env.perturbation, agent=self.name, seed=env.seed, interventions=interventions, true_failures=failures, recovery_attempts=recovery_attempts, recovery_successes=int(state.success and recovery_attempts > 0), event_log=env.events)


class SelfHealingAgent(BaseAgent):
    name = "self_healing"

    def __init__(self, bundle: ModelBundle, *, memory: RecoveryMemory | None = None, config: AgentConfig | None = None) -> None:
    ...
    def run_episode(self, env: TabletopManipulationEnv) -> EpisodeResult:
        state = env.reset()
        plan = deque(env.nominal_plan(state))
        steps = interventions = detections = tp = fp = recovery_attempts = recovery_successes = 0
        ...
        while steps < self.config.max_steps and not state.success:
            ...
            diverged = (not result.action_succeeded) or (not recovery_action and score > self.bundle.residual_threshold)
            if diverged:
                detections += 1
                interventions += 1
                eligible_failure_steps = sorted({int(e["step"]) for e in env.events if e.get("failure") and 0 <= state.step_index - int(e["step"]) <= self.config.detection_window and int(e["step"]) not in matched_failure_steps}, reverse=True)
        ...
        failures = len({int(e["step"]) for e in env.events if e.get("failure")})
        return EpisodeResult(success=state.success, steps=steps, perturbation=env.perturbation, agent=self.name if self.memory is None else "self_healing_memory", seed=env.seed, interventions=interventions, true_failures=failures, detections=detections, true_positive_detections=tp, false_positive_detections=fp, recovery_attempts=recovery_attempts, recovery_successes=recovery_successes, event_log=env.events)
